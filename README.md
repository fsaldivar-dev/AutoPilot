...
> **Nota**: el motor iOS default ya NO usa las APIs de Accesibilidad de macOS (robaban el foco del usuario en cada accion — ARD #164). `auto launch` inyecta el observer in-process (~3.6ms/tap) y el fallback es el runner XCUI dentro del simulador. El permiso de Accesibilidad solo hace falta si fuerzas el modo legacy (`AUTO_FORCE_AX=1` / `AUTO_BRIDGE=simulator|hybrid`) o para el recorder (CGEventTap). [Historia del path AX en Cap. 12](docs/libro/12-permisos-accesibilidad.md)
