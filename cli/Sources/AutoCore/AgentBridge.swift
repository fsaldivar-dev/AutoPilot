...
public final class AgentBridge: DeviceBridge {
    ...
    private func sendCommandInternal(_ method: String, params: [String: Any]?, allowRecover: Bool) throws -> Any {
        ...
        guard !responseData.isEmpty else {
            ...
            throw BridgeError.agentFailed("Empty response from agent (agent may have crashed)")
        }
    ...
    private func createSocket() throws -> Int32 {
        ...
        guard result == 0 else {
            close(sock)
            throw BridgeError.agentFailed("Cannot connect to agent at \(host):\(port). Is the agent running?")
        }
